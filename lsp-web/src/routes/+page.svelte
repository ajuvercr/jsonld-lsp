<script lang="ts">
  ...
  let value = `{
  "@context": "https://data.vlaanderen.be/doc/applicatieprofiel/sensoren-en-bemonstering/kandidaatstandaard/2022-04-28/context/ap-sensoren-en-bemonstering.jsonld",
  "@id": "you",
  "test": { "@id": "me" }
}`;
  ...
  function do_set_diags(msg: Diagnostics) {
    console.log("DIAGS: ", msg.diagnostics.length);
  ...
  async function localCompletionSource(
    ...
    console.log("node before", nodeBefore.type);
  ...
  const linterExtension = linter((view) => {
    console.log("linter extension ", diagnosticsBuffer.length);
